...
if [[ $SARINDEX = 1 ]]; then
    ./vendor_cmd_tool -f /vendor/etc/wifi/sar-vendor-cmd.xml -i wlan0 --START_CMD --SAR_SET --ENABLE 1 --NUM_SPECS 0 --END_CMD
elif [[ $SARINDEX = 3 ]]; then
    ./vendor_cmd_tool -f /vendor/etc/wifi/sar-vendor-cmd.xml -i $WLAN_INGERFACE --START_CMD --SAR_SET --ENABLE 3 --NUM_SPECS 0 --END_CMD
